...
def lesson_prompt_context(state: dict[str, Any] | None) -> dict[str, str]:
    ...
    if not topic_label:
        ...
        if support_mode == "confused":
            ...
            choices = suggestion_labels[:3] or [suggested_topic]
            if len(choices) > 1:
                choices_text = ", ".join(choices[:-1]) + " или " + choices[-1]
            else:
                choices_text = choices[0]
            instruction = (
                "Урок еще не выбрал тему. Тепло поздоровайся и предложи ребенку выбрать одну тему: "
                f"{choices_text}. Спроси одним коротким дружелюбным вопросом, что интереснее. "
                "Пока НЕ учи конкретное английское слово и не давай задание — сначала дай выбрать тему."
            )
